...
版本：1.5｜日期：2026-09-12｜狀態：符合目前Core程式，供三方確認與串接
...
同一discussion的世界版本固定。資料不同或有新事件時，Core使用新discussion，不混用舊回覆。專家不得自行更新世界、增加資源、執行世界操作工具或修改規則；可執行本地唯讀查規則、RAG與單tick算術核算。沒有預測Simulator，不能宣稱建議已經試跑。
...
本版外層沒有schema_version、type、in_reply_to或rules_version欄位，請勿自行增加，以免目前接收器拒絕。規則版本讀 `content.rules.rules_version` 與 `content.world.rules_version`。本文1.5是協定文件版本，不是現有傳輸欄位。
...
- world.crew[]：id、food_energy（遊戲kcal）、water（L）；alive為必填布林值，必須來自實際世界狀態。
...
```json
{
  "discussion_id": "a5ff4eaa-2ac2-446c-9160-583a9cf5cc7d",
  "round": 1,
  "sender": "core",
  "recipient": "plant",
  "world_version": 0,
  "content": {
    ...
    "rules": {
      ...
      "electricity_reference": {
        "kWh_per_EU": 3.9745,
        "plot_area_m2": 5,
        "power_kW_per_plot": 0.8280434782608697,
      ...
      "irrigation": {
        "water_per_plot": 8.7,
        "power_per_plot": 0.20833903088712283,
        "atomic_inputs": true,
  ...
  "message_id": "f939052c-495d-4bb8-a71d-be70602a5146",
  "explanation": {
    "observations": [],
    "proposals": [],
    "reviews": [],
...
```json
{
  ...
  "content": {
    "observations": [
      "五種作物各4塊，目前生長進度均為0。",
      "20塊地完整灌溉每tick需要174 L水與4.16678061774 EU電力。"
    ],
    "priorities": [
      "crew存活並維持資源循環"
    ],
...
```json
{
    ...
    "suggested_actions": [
      {
        ...
        "description": "在個人基礎消耗後安排合適發電量，並在後續tick的補給階段安排進食／飲水。"
      }
    ],
    "acceptable_tradeoffs": [
      "工作量增加會縮短下一次進食前的餘裕。"
  ...
  "explanation": {
    ...
    "proposals": [
      {
        "proposal_id": "human-proposal-001",
        "strategy": "在個人基礎消耗後安排合適發電量，並在後續tick的補給階段安排進食／飲水。",
...
world與crew／plot允許後端附加狀態；resources固定四個欄位，不接受拼錯或自行增加的資源名稱。`alive`、`world_status`、`snapshot_phase`均必填，不可由Core猜測或補預設值。schema驗證不代表世界仍存活：0是合法數值，但死亡效果仍由世界規則執行。
    ...
    "crew": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "alive": {
            "type": "boolean",
            "description": "必填，由世界後端提供實際存活狀態，不可推測或補預設值。"
          }
        },
        "required": [
          "id",
...
本次只擴充world／crew事實欄位，訊息外層與explanation七欄schema維持v1.3。Human回報已提供POST /discuss；其測試是Human端結果，Core尚未進行跨服務驗收。

### 快照與時間

- world.world_status必填：running／paused／planning／error_paused／failed，原樣傳送實際狀態。
- world.snapshot_phase必填between_ticks：世界後端必須在完整tick之間原子性取得快照；填上字串本身不能保證一致性。
- 四名crew的alive各自必填boolean，不能从個人資源數字推測。
- 世界預設running、時間持續走；Core背景觀察／評估不暫停。重規劃時才取得Core擁有的暫停，成功提交後恢復。玩家暫停或錯誤暫停不可被自動覆蓋。
- failed或任一crew已死亡時，Core停止規劃，不把狀態改成planning送出。範例running、alive=true只是明示fixture設定。
- world_status取代舊控制器status；若後端仍附status，必須與world_status一致。缺欄位會拒絕，不自動使用running補值。

### 可選候選計畫

第一階段可不提供world.next_tick_plan，Human仍可給需求與策略建議。Core目前不會把執行中plan、previous_messages或多階段plan自動映射成此候選。

如由整合端明確提供，next_tick_plan必須對應同快照的下一tick：

| 欄位 | 格式 |
|---|---|
| for_tick | world.tick + 1 |
| crew_tasks | 四人各一筆，含crew_id與task；task為idle/eat/drink/generate/plant/harvest/clear；eat/drink附amount，generate附work_fraction，作物工作附plot_id，plant另附crop_type |
| refill_order | 只列出所有eat/drink的crew ID，依執行順序 |
| water_production_request_liters | 非負請求量 |
| irrigation_allocations | null代表未知；[]代表明確不分配；否則依順序列出plot_id、water_quota_liters、power_quota_eu |
| crop_operation_order | 只列出所有作物工作的crew ID，依執行順序 |

world.water_production_available可為實際boolean，未知可省略或null；不能推測為true。這兩個選填欄位若存在會原樣傳送，Core目前不驗證Human Plan內部DTO，也不直接執行它。正式候選映射需依Human完整DTO再實作。

### 既有規則確認與建議的使用

發電比例公式已與主spec第6.2節相符：

```text
a_i = min(request_i, 1, E_after_base_i / 100)
W = min(sum(a_i), O2 / 25, power_headroom / 250)
w_i = W * a_i / sum(a_i)
```

sum(a_i)=0時全員w_i=0。工作耗盡個人能量或公共氧氣仍立即死亡，比例縮減不是保命保證；部分工作仍占用整個tick。補給先於基礎消耗，同tick後續製水不能回頭供飲水使用。

Human本地核算不是世界執行，也不是長期Simulator。禁止以known_after_values覆寫世界；其範圍止於灌溉後、作物操作前。feasible_in_scope、reviews的accept／modify都不代表整體安全或已執行；仍須看will_die、critical、依賴及核算範圍，再由Core決策、後端驗證。

Human回報第二次失灌仍標warning；本世界規格第二次為critical，第三次死亡。Core不依Human總結降級自身灌溉警報。這是Human待修正的報告分級，不改世界規則。

### 文獻電力映射補充

已採用Plant設備基準：1 EU = 3.9745 kWh；設備電力按世界1 tick＝1小時，不能套作物加速。每塊EU/tick＝((250×5÷2.3×1.45＋8×5)÷1000)×tick_hours÷3.9745。20塊397.460869565217 kWh/day，約4.16678061774 EU/tick。規則版本更新greenhouse-2026-09-12-v2，取代原5 EU/塊/tick；水、產氧、收穫及人類係數不變。Human不負責種植計算，應容忍植物metadata更新；若核算含種植的候選，需使用傳入新配額，不得硬編舊5 EU。

### 串接限制

Human開發入口尚無token或HTTPS。Core目前HTTPSpecialist只允許HTTPS與localhost／127.0.0.1／::1的HTTP；同機可直接測試，其他電腦的明文HTTP尚未啟用。需取得實際可達URL後決定部署或明確開發設定，不能把本機127.0.0.1當成另一台電腦。

HTTP 422為格式／事實缺漏、409為契約／規則不符、502／504為服務失敗／逾時、500為未預期錯誤；Core目前對非2xx一律保持規劃失敗，不自動重試或採用錯誤body的baseline。HTTP 200只代表成功取得建議。
